...
def test_analisis_goles_mitad_csv(datos, encuentros, output_file):
    ...
    with open(output_file, 'w', newline='', encoding="utf-8") as csvfile:
        ...
        csv_writer.writerow(["Probabilidad1", "Cuota", "Probabilidad2", "Local", "Visitante", "Fecha"])

        # Obtener los resultados de la función analisis_goles
        resultados = list(analisis_goles_mitad(datos, encuentros, n=1, f=6, p=0.90,supera_n=True))
        ...
        for elemento in elementos:
            if elemento.is_displayed():
                ...
                for corner in corners:
                    ...
                    corner_number = re.search(r'\d+', corner_text)
                    if corner_number:
                        corner_number = corner_number.group()
                ...
                if not (all(el == '90' for el in home_elementos + away_elementos)):
...
# Crear un conjunto para almacenar los valores únicos de 'Local' en resultados.csv
    ...
    with open(f'../data/TestLista({fecha_global}).csv', 'r', encoding='utf-8') as resultados_file:
        ...
        for line in resultados_file:
            ...
            local_set.add(valores[3])  # Suponiendo que 'Local' es la cuarta columna en resultados.csv
    ...
    with open(f'../data/Resultados1({fecha_global}).csv', 'w', newline='', encoding='utf-8') as output_file:
        fieldnames = ['Probabilidad1', 'Cuota', 'Probabilidad2', 'Local', 'Visitante', 'Fecha', 'GolesLocal', 'GolesVisitante', 'GolesTotales', 'Acertada']
...
if __name__ == '__main__':
    fecha_inicial = datetime(2024,1,1) #año,mes,dia
    fecha_actual = datetime.now()
    delta = timedelta(days=1)
    f = 6

    while fecha_inicial <= fecha_actual:
        fecha_global = fecha_inicial.strftime('%d-%m-%Y')
        start_time = time.time()
        
        try:
            process(fecha_global)
            end_time = time.time()
            total_time = end_time - start_time
            print(f"Fecha: {fecha_global}, Tiempo total: {total_time} segundos")
        except FileNotFoundError:
            print(f"Archivo no encontrado para la fecha {fecha_global}. Saltando...")
        except ValueError as e:
            print(f"Error procesando la fecha {fecha_global}: {e}. Saltando...")
