...
def process_for_date(fecha_global):
    ...
    DatosPartidosMinutosGoles = f'../data/DatosPartidosMinutosGoles({fecha_global}).csv'
...
# Crear un conjunto para almacenar los valores únicos de 'Local' en resultados.csv
    ...
    with open('analisis_filtrados.csv', 'w', encoding='utf-8') as salida_file:
        ...
        with open(encuentros, 'r', encoding='utf-8') as encuentros_file:
    ...
    urls = []
    with open(DatosPartidosMinutosGoles, mode='w', newline='', encoding="utf-8") as csv_file:
    ...
    equipos_validos = set()
    with open(DatosPartidosMinutosGoles, 'r', encoding='utf-8') as minutos_goles_file:
...
if __name__ == '__main__':
    ...
    # Define la fecha de inicio y fin para el bucle
    fecha_inicio = datetime.datetime.strptime("01-09-2023", "%d-%m-%Y").date()
    fecha_fin = datetime.datetime.strptime("17-09-2023", "%d-%m-%Y").date()

    # Define un paso negativo para retroceder un día en cada iteración
    paso = datetime.timedelta(days=-1)

    fecha_actual = fecha_inicio

    while fecha_actual >= fecha_fin:
        # Convierte la fecha actual a un formato de cadena "dd-mm-yyyy"
        fecha_global = fecha_actual.strftime("%d-%m-%Y")

        print(f"Procesando para la fecha: {fecha_global}")

        # Llama a tu función principal para procesar los datos
        process_for_date(fecha_global)
